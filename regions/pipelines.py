...
class RegionsPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `regions`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM regions WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO regions (date, title, link, content)"
